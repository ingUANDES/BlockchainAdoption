...
% --- METADATA ---
\title[Financial Networks]{Financial Networks \& Technological Innovation: \\ A Dual-Network Model}
...
\begin{frame}{Motivation: The Regulatory Constraint Problem}
    ...
    \textbf{The Problem:}
    \begin{itemize}
        \item In financial networks, agents benefit from trading with other agents
        \item Regulations may prohibit specific direct links
        \item \textbf{Consequence:} welfare loss
    \end{itemize}
    
    \textbf{Main Question}
    \begin{itemize}
        \item Can \textcolor{blue}{alternative infrastructures} help recover this loss?
    \end{itemize}
    
    \textbf{Questions:}
    \begin{itemize}
        \item Can the addition of a new agents solve this?
...
\vspace{0.3cm}
We extend the model to include a second network (alternative infrastructure) that competes with the traditional network in transparency and cost dimensions.
\end{frame}
...
\textbf{Optimal portfolio solution (Theorem 1 Jalan et al (2024)):}
$$w_i^* = Q_i(\mu_i - Pe_i)$$
...
\begin{frame}{Extension: Adding Competition (Fintech Entry)}
    ...
    \end{columns}
    High correlation ($\rho = 0.99$) between Fintech and Local Bank ensures they are seen as close competitors. 
\end{frame}


\begin{frame}{Solution 1: Add Competition (Fintech Entry)}
    \textbf{Logic:} Competition should central agent rents and improve efficiency
...
\begin{frame}{Why Competition Fails?}
    \begin{columns}
        \column{0.5\textwidth}
...
\begin{frame}{Justification for Quadratic Costs}

    % PARTE 1: La evidencia matemática del paper original
    The original model introduces friction $F_i(w_i)$ in the utility function to represent regulatory constraints (Eq. 2):
    \begin{equation}
         g_i(W,P) = w_i^T(\mu_i - Pe_i) - \gamma_i w_i^T \Sigma_i w_i - \textcolor{blue}{\mathbf{F_i(w_i)}}
    \end{equation}
        
    
    \begin{block}{\textbf{Theorem 6 (Jalan et al., 2024)}}
        For a unique stable network equilibrium to exist, the friction cost function $F_i: \mathbb{R}^n \to \mathbb{R}$ must be  twice differentiable and the gradient $\nabla F_i(w)$ consists of strictly increasing functions.
        There must exist strictly increasing functions $f_{ji}$ such that:
        $$ \nabla F_i(w) = [f_{1i}(w_1), \dots, f_{ni}(w_n)]^\top $$
        Then, there exists a unique stable point.
    \end{block}
    

    \begin{itemize}
        \item Ensure uniqueness and existence of a stable point
        \item Margin requirements, network congestion (gas fees)
    \end{itemize}
...
\begin{frame}{Distributional Effects: Who Benefits?}
    ...
    \begin{columns}
        ...
        \column{0.45\textwidth}
        \centering
        \textbf{Welfare Gain Distribution}
        
        \vspace{0.2cm}
        
        \begin{tikzpicture}
            % Legend
            \fill[SafeGreen!80] (0,0.8) rectangle (0.3,1.0);
            \node[right, font=\scriptsize] at (0.35,0.9) {Local Firm};
            \fill[DeepBlue!60] (2.2,0.8) rectangle (2.5,1.0);
            \node[right, font=\scriptsize] at (2.55,0.9) {National Bank};
            \fill[DeepBlue!40] (4.4,0.8) rectangle (4.7,1.0);
            \node[right, font=\scriptsize] at (4.75,0.9) {Others};
            
            % Stacked bar (total width = 6cm)
            \fill[SafeGreen!80] (0,0) rectangle (5.058,0.5);  % 84.3% of 6cm
            \fill[DeepBlue!60] (5.058,0) rectangle (5.718,0.5);  % 11.0% of 6cm
            \fill[DeepBlue!40] (5.718,0) rectangle (6,0.5);  % 4.8% of 6cm (combined)
            
            % Percentage labels
            \node[font=\scriptsize, white] at (2.5,0.25) {84.3\%};
            \node[font=\scriptsize, white] at (5.388,0.25) {11\%};
            \node[font=\scriptsize] at (5.859,0.25) {\tiny 4.8\%};
            
            % Bottom label
            \node[font=\scriptsize] at (3,-0.3) {Share of Welfare Gain (\%)};
        \end{tikzpicture}
    \end{columns}
\end{frame}
...
\begin{frame}{Main Conclusions}
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \item Persistent 40.6\% welfare gap
        \end{itemize}
