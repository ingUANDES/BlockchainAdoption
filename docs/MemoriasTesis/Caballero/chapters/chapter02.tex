...
al crecer $n$ —$\lceil n \log n \rceil$ crece con $n$, aunque sea
moderadamente—, sino que crece muy poco comparado con lo que exigiría estimar
...
logaritmo —un requisito de datos modesto, no las decenas o cientos que una
...
$t\in[T]$ —aversión al riesgo y covarianza homogéneas y constantes en el
...
que la inferencia de $\Sigma$ sea tratable —compatible con la decisión de
alcance de este trabajo (Capítulo~\ref{c1}) de no abordar heterogeneidad en
$\gamma_i$ por ahora.
...
contrato observado —los montos de financiamiento entre BID/CORFO, la
...
convexidad del problema —relevante si el perfil de riesgo del programa cambió
estructuralmente durante 2016--2024 (p.ej. tras 2020).
